...
\texttt{--project} and repeatable \texttt{--repo} values, then calls
\texttt{CgsDocument.from\_project\_definition()}; it owns no repository grammar
or normalization. Loading an equivalent \texttt{.cgs} file and using the CLI
definition produce semantically identical canonical documents.
...
src/ComplexGitSync/
  ...
  cgs_format.py                CgsDocument (.cgs parse/normalize/validate)
