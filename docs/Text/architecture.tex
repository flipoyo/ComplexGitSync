...
\texttt{WorkingGitTree}, which holds a mutable
\texttt{WorkingRepo} per node.  Registry entries are indexed by a
...
LEAF}; the project root repository is pulled before child submodules are
updated from their parent repositories.
...
For \texttt{.cgs} refs, repositories can be declared by branch or by tag.  If
both are declared for the same repo, \texttt{validate} enforces hash
equivalence and raises
\texttt{incompatibilities between branch (hash) and tag(val) in .cgs} when they
differ.
...
Provider handling is centralized in \texttt{git\_repo.py}: the
\texttt{GitProvider} enum, canonical provider set, known-host map,
repository-identifier parser, and \texttt{RepoAddress} URL composer are shared
by authoring and runtime code. GitHub, GitLab, and Codeberg map respectively to
\texttt{github.com}, \texttt{gitlab.com}, and \texttt{codeberg.org}.
\texttt{custom} has no known-host entry and requires an explicit
\texttt{gitprovider\_url}; no fallback host is guessed.
...
remotes, branch divergence, unresolved merges, missing submodule links, and
stale recorded \texttt{commit\_sha} values. Diagnostics are severity-based:
...
src/ComplexGitSync/
  ...
  cgs.py                       CgsDocument (.cgs parse/normalize/validate)
...
\texttt{config\_document.py}; \texttt{cgs.py} owns the complete
