...
When \texttt{--output-dir} is omitted, \cgscli{} uses the workspace-level
default \texttt{CGSHOME=../..}. The explicit equivalent is:

\begin{lstlisting}[language=bash]
$ pixi run cgitsync initialise examples/complexgitsync.cgs --output-dir "$CGSHOME"
\end{lstlisting}
...
default is \texttt{../..}, and later commands can also discover the workspace
by walking upward from the current directory.
