...
\texttt{GT-LOAD->GT-DISCOVER->GT-VALIDATE->GT-CLONE}, the explicit workflow
\texttt{load->expand->validate->clone}, the per-repository \texttt{git clone}
action, the resolved root path, and a compact tree outline. Structured log
records also put the operation code first, for example \texttt{GT-DISCOVER},
\texttt{GT-VALIDATE}, \texttt{FS-PURGE}, or \texttt{GT-CLONE}.
...
\texttt{GT-LOAD->GT-DISCOVER->GT-VALIDATE->FS-PURGE->GT-CLONE}; the matching
workflow is \texttt{load->expand->validate->purge->clone}.  The purge phase can
...
\begin{lstlisting}[language=bash]
$ pixi run cgitsync view-tree .cgitsync/state/complexgitsync.gts
$ pixi run cgitsync view-tree .cgitsync/state/complexgitsync.gts --depth 2 --collapse parent_2
$ pixi run cgitsync view-operation .cgitsync/state/complexgitsync.gts
\end{lstlisting}
