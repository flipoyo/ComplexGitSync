...
\begin{lstlisting}[language=bash]
$ cgitsync initialise <spec.cgs> [--output-dir DIR]
...
\begin{itemize}
  \item \texttt{.cgs} source (clone mode): clones the full repository tree
    (see \texttt{clone} below). The optional \texttt{--output-dir} controls
    the workspace-level output path. When omitted, it defaults to
    \texttt{CGSHOME=../..}.  On success, ends in \texttt{READY}.
...
the \texttt{initialise(.cgs)} output-dir default is \texttt{../..}; later
