...
\begin{itemize}
  ...
  \item Direct CLI authoring: requires \texttt{--project} and at least one
    ...
    The CLI collects values only; \texttt{cgs\_format.py} parses, normalizes,
    and validates them into the same canonical \texttt{CgsDocument} produced
    from an equivalent \texttt{.cgs} file.
\end{itemize}
