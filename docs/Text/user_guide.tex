...
The integration suite validates the command cycle in this order with local
repositories and no network dependency:
\texttt{add -> commit -> push -> tag -> freeze}. The CLI path is exercised
first (terminal-oriented usage), and the equivalent Python API git dispatch is
