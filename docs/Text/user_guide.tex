...
The project-local \texttt{.lgr} register is now active; remaining roadmap work
tracks ledger evolution (\texttt{T32}).
...
branches, existing remotes, no pre-existing tag of the same name, and
parent/child repositories linked as Git submodules. Tag creation is non-forcing
by default (no implicit \texttt{-f}).
...
checks remote availability, branch alignment, tag absence, and submodule links
for all parent/child repository boundaries.
...
The integration suite validates local no-network lifecycle coverage:
\texttt{clone\_cgs -> launch\_release} and the READY git command cycle
\texttt{add -> commit -> push -> tag -> freeze}. The CLI path is exercised
first (terminal-oriented usage), and the equivalent Python API git dispatch and
launch/restore flows are covered in the same suite.
...
Orchestration plans are available through the Python API:
\texttt{ComplexGitSyncClient.orchestrate(<plan.goc>)}.
...
\textbf{Top-level sections:}
\begin{itemize}
  \item \texttt{[document]} — \texttt{format\_version},
        \texttt{schema\_version}, \texttt{generated\_at},
        \texttt{command\_origin}, \texttt{hash\_algorithm},
