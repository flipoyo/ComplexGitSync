...
The CLI exposes four command groups once a \texttt{GitTree} is \texttt{READY}.
...
and \texttt{import-submodules}.  The configuration level, \texttt{discover},
\texttt{configure} and
\texttt{create-cgs}, authors a \texttt{.cgs} specification without requiring
existing state.  The memory level, \texttt{remember}, \texttt{memorize},
\texttt{retrieve}, and \texttt{reload}, binds a project to an external
SSH-Git \emph{Memory} repository (distinct from the project's own remotes)
and persists or restores a finalized local Memory State.  Redundant
...
\begin{lstlisting}[language=bash]
$ cgitsync initialise <spec.cgs> [--output-path DIR] [--commit-gitignore]
    [--force-gitignore-sync] [--git-user-name NAME] [--git-user-email EMAIL]
$ cgitsync initialise <snapshot.gts>
$ cgitsync initialise --project NAME --repo PROVIDER:OWNER/REPO [--repo ...] [--output-path DIR]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync bootstrap <spec.cgs> <project_name> [--cgs-path DIR]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync discover [ROOT] [--write FILE] [--max-depth N]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync configure [--output FILE]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync clean-init <spec.cgs> [--output-path DIR] [--commit-gitignore]
    [--force-gitignore-sync] [--git-user-name NAME] [--git-user-email EMAIL]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync pull [spec.cgs|snapshot.gts] [--commit-gitignore]
    [--force-gitignore-sync] [--git-user-name NAME] [--git-user-email EMAIL]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync import-submodules REPO_ROOT [--apply] [--output FILE]
\end{lstlisting}
...
\begin{lstlisting}[language=bash]
$ cgitsync remember <spec.cgs> [--output-path DIR] [--service HOST] [--remote-name NAME]
\end{lstlisting}

Binds the given \texttt{.cgs} artefact to its external SSH-Git Memory
endpoint, resolving \texttt{CGSHOME} the same way \texttt{initialise} does.
\texttt{--service} defaults to \texttt{forge43.io}; \texttt{--remote-name}
defaults to \texttt{forge43}.

\subsection{\texttt{memorize}}

\begin{lstlisting}[language=bash]
$ cgitsync memorize <memory-state-dir> [--branch BRANCH]
\end{lstlisting}

Persists a finalized local Memory State directory
(\texttt{.cgitsync/state(<hash>)\_i/}) to the configured SSH-Git Memory
remote, on \texttt{--branch} (default \texttt{main}).

\subsection{\texttt{retrieve}}

\begin{lstlisting}[language=bash]
$ cgitsync retrieve <name> [--output-path DIR] [--branch BRANCH] [--service HOST] [--remote-name NAME]
\end{lstlisting}

Retrieves the named external SSH-Git Memory repository into a clean
\texttt{CGSHOME} (\texttt{CGSPATH/<name>}; defaults to \texttt{\$CGSHOME} or
the current directory when \texttt{--output-path} is omitted).

\subsection{\texttt{reload}}

\begin{lstlisting}[language=bash]
$ cgitsync reload <name> [--output-path DIR] [--branch BRANCH] [--service HOST] [--remote-name NAME]
\end{lstlisting}

Retrieves the named external Memory repository, then restores the
ComplexGitSync execution context from it — the recovery path for a
\texttt{CGSHOME} that only has its Memory binding left.
...
The repository also keeps two checked-in tutorial case tests for this
authoring surface: \texttt{docs/tutorial\_cgsi1.md} walks a fully exercised
local sandbox topology, while \texttt{docs/tutorial\_cawaqs.md} documents the
larger CaWaQS onboarding tree and the point where \cgscli{} hands off to the
project's existing \texttt{make}-based build workflow.

\texttt{examples/cawaqsviz.cgs} is a third, real-world (not synthetic)
example, describing
\url{https://gitlab.com/cawaqs/gviz/cawaqsviz}. It exercises several of the
patterns above at once: a nested GitLab subgroup path
...
against the live repositories) as \texttt{planning/Onboarding\_DevPlanTicket.md}
Phase~1. See \texttt{docs/tutorial\_cgsi1.md} \S5 for the full worked
example, and \texttt{bootstrap} above for running \cgspkg{} against it
without cloning \cgspkg{} inside the tree it manages.
