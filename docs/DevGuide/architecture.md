...
headers and that `AgentSpecs/audit.md` states as rules.

**Why it exists.** The isolation refactor
(`AgentSpecs/archive/20260828_Isolation_DevPlanTicket.md`) split
...
module set actually depends on itself. `AgentSpecs/audit.md`'s ring table
is dev-facing but text-only — no diagram. This file is that diagram, plus
the one explicit Tier↔Ring reconciliation
`AgentSpecs/20260831_DocRewritePlanTicket.md` §1.1 calls for.
...
condensed pointer to `AgentSpecs/audit.md`'s four import rules and ceiling
ratchet (§4) — `AgentSpecs/audit.md` stays authoritative for those, this
...
```mermaid
graph TD
    AUDIT["AgentSpecs/audit.md<br/>ring table + import rules<br/><i>enforced source of truth</i>"] --> ARCH["architecture.md<br/><i>YOU ARE HERE</i>"]
...
`AgentSpecs/audit.md` describes **five Rings** — an *import-direction/I/O-boundary*
...
[`AgentSpecs/audit.md`](../../AgentSpecs/audit.md) — this is a condensed
