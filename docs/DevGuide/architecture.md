...
headers and that `AGENT.md` states as rules.
...
module set actually depends on itself. `AGENT.md`'s ring table is
dev-facing but text-only — no diagram. This file is that diagram, plus the
one explicit Tier↔Ring reconciliation
...
condensed pointer to `AGENT.md`'s four import rules and ceiling ratchet
(§4) — `AGENT.md` stays authoritative for those, this file only summarizes.
...
```mermaid
graph TD
    AGENT["AGENT.md<br/>ring table + import rules<br/><i>enforced source of truth</i>"] --> ARCH["architecture.md<br/><i>YOU ARE HERE</i>"]
...
`AGENT.md` describes **five Rings** — an *import-direction/I/O-boundary*
...
Full text and rationale live in [`AGENT.md`](../../AGENT.md) — this is a
condensed pointer, not a restatement:
