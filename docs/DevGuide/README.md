...
`AgentSpecs/AdditionalSpecs.md` and each module's own docstring header, invisible to
...
`AgentSpecs/AdditionalSpecs.md`, which stays the enforced, authoritative
...
| File | Covers |
|---|---|
| [`architecture.md`](architecture.md) | The Ring model, the Tier↔Ring mapping, a per-ring module dependency graph, a one-row-per-module contract table, and a condensed pointer to `AgentSpecs/AdditionalSpecs.md`'s import rules and ceilings. |
