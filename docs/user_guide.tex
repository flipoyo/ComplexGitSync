...
This guide follows the canonical lifecycle:
...
\begin{enumerate}
  ...
  \item \texttt{add}
  \item \texttt{git(tree, "commit", msg)} $\rightarrow$ commits all repos leaf-first
  \item \texttt{git(tree, "push")} $\rightarrow$ update hash in \texttt{GitTree}
  \item \texttt{git(tree, "tag", name)} $\rightarrow$ update tag in \texttt{GitTree}
  \item \texttt{freeze} $\rightarrow$ emit the next \texttt{.gts} id
\end{enumerate}

Backward-compatible aliases: \texttt{read} $\rightarrow$ \texttt{load},
\texttt{verify} $\rightarrow$ \texttt{validate},
\texttt{tree} $\rightarrow$ \texttt{expand} (CLI, with runtime-snapshot preference).
The commands \texttt{describe} and \texttt{restart} remain compatibility aliases of
\texttt{print} and \texttt{pull} respectively.
...
\begin{lstlisting}[language=bash]
$ cgitsync load <spec.cgs>
\end{lstlisting}

Parses the \texttt{.cgs} file and builds the dependency-tree registry
(lifecycle step~1, \texttt{LOADED} state).  Prints a one-line state summary.

\subsection{\texttt{expand}}

\begin{lstlisting}[language=bash]
$ cgitsync expand <spec.cgs|snapshot.gts>
\end{lstlisting}

Loads the source, runs nested \texttt{.cgs} discovery from parents to leaves
(recursive), and renders the dependency graph as a text tree (lifecycle
step~2, \texttt{PENDING} state).  The canonical step-2 command.

\subsection{\texttt{validate}}

\begin{lstlisting}[language=bash]
$ cgitsync validate <spec.cgs|snapshot.gts>
\end{lstlisting}

Parses the source, recomputes the tree lifecycle state, and prints a one-line
summary.  Exits~0 on success, non-zero on parse or validation error (lifecycle
step~3, checks \texttt{READY} state).

\textbf{Output fields:} \texttt{lifecycle\_state}, \texttt{ready},
\texttt{complete}.
...
Backward-compatible alias for \texttt{expand}.  When a newer runtime snapshot
is registered for the given \texttt{.cgs} file, \cgscli{} loads that
...
This is a compatibility alias of \texttt{print}; both commands share the same
implementation.
...
This remains a compatibility helper; the planned lifecycle name is
\texttt{pull}.
...
Resynchronises the project tree using lifecycle-compatible semantics:
\texttt{.cgs} sources follow the historical \texttt{restart} flow, while
...
This helper is not treated as a primary lifecycle step; conceptually it is a
checkout from recorded \texttt{.gts} state.
