...
\texttt{tree} for the \texttt{expand} stage, \texttt{validate} for the
\texttt{verify} stage, and \texttt{describe} as the nearest current
\texttt{print(.gts)} command.

The remaining planned work is \texttt{print(.gts)}, \texttt{pull(.gts)},
\texttt{orchestrate(.goc)}, and project-local \texttt{.lgr} register
management.
...
This is the nearest current compatibility command for the planned
\texttt{print(.gts)} step.
...
\begin{itemize}
  ...
  \item \texttt{print(.gts)} — planned user-facing name for saved-state
    inspection.
  \item \texttt{pull(.gts)} — planned user-facing name for resynchronisation
    from recorded state.
