...
This guide follows the project lifecycle strictly:

\begin{enumerate}
  \item \texttt{read(.cgs)} $\rightarrow$ \texttt{.gts LOADED}
  \item \texttt{expand(.gts, LOADED)} $\rightarrow$ \texttt{.gts PENDING}
  \item \texttt{verify(.gts, PENDING)} $\rightarrow$ \texttt{.gts READY}
  \item \texttt{clone(.gts)}
  \item \texttt{checkout(.gts)}
  \item \texttt{add}
  \item \texttt{commit}
  \item \texttt{push} $\rightarrow$ update hash in \texttt{GitTree}
  \item \texttt{tag} $\rightarrow$ update tag in \texttt{GitTree}
  \item \texttt{freeze} $\rightarrow$ emit the next \texttt{.gts} id
\end{enumerate}

Current command names still expose some historical compatibility labels:
\texttt{tree} for the \texttt{expand} stage, \texttt{validate} as a
compatibility alias of \texttt{verify}, \texttt{describe} as a compatibility alias of
\texttt{print}, and \texttt{restart} as a compatibility alias of
\texttt{pull}.
...
\begin{lstlisting}[language=bash]
$ cgitsync verify <spec.cgs>
\end{lstlisting}

Parses the \texttt{.cgs} file, builds the dependency-tree registry, and
verifies every repository is loaded before readiness checks. Exits~0 on success,
...
In lifecycle terms this is the current compatibility command for the
\texttt{expand} stage.
