...
\begin{itemize}
  \item \texttt{clone\_cgs(...)} bootstraps a tree and must finish in
    \texttt{READY}.
  \item \texttt{checkout(...)} requires \texttt{READY} and keeps
    \texttt{READY}.
  \item \texttt{commit(...)}, \texttt{push(...)}, \texttt{tag(...)}, and
    \texttt{freeze\_release(...)} are \texttt{READY}-gated mutation methods.
  \item \texttt{launch\_release(...)} loads a release snapshot and must restore
    the tree to \texttt{READY}.
\end{itemize}
