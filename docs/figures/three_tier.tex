...
\node[tier, fill=green!10, draw=green!40] (actions) at (0,-2.3) {
  ...
  \texttt{read} \ $\cdot$ \ \texttt{tree} \ $\cdot$ \ \texttt{validate} \ $\cdot$
  \ \texttt{clone} \ $\cdot$ \ \texttt{restart / pull} \ $\cdot$ \ \texttt{checkout}\\
  \texttt{add} \ $\cdot$ \ \texttt{commit} \ $\cdot$ \ \texttt{push} \ $\cdot$
  \texttt{tag} \ $\cdot$ \ \texttt{freeze} \ $\cdot$ \ \texttt{launch}\\
