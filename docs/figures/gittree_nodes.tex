...
\node[rootnode] (root) at (0,0) {
  \textbf{root GitRepo}\\NodeType = ROOT
};

% Two direct children of root
\node[parentnode] (parent) at (-3,-2) {
  \textbf{nested GitRepo}\\NodeType = PARENT
};
\node[leafnode] (lib) at (3,-2) {
  \textbf{leaf GitRepo}\\NodeType = LEAF
};
...
% Annotations
\node[label, right=0.3cm of root] {entry point from the root .cgs};
\node[label, right=0.2cm of parent] {expanded from nested .cgs discovery};
\node[label, right=0.2cm of lib] {terminal repo in the GitTree};
