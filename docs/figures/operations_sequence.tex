% operations_sequence.tex — TikZ figure showing the add/checkout/commit/push flow
...
\node[seclabel] (ad-title) at (5.5, 0) {\texttt{add\_tree}};

\node[gatebox] (ad-gate) at (5.5,-1.0) {assert READY};

\node[step] (ad-stage) at (5.5,-3.25) {
  \textbf{git add --all}\\
  each repo\\(leaf-first)
};

\node[orderbox] (ad-order) at (5.5,-7.2) {leaf → parent → root};
...
\node[seclabel] (cm-title) at (10.8, 0) {\texttt{commit\_tree}};

\node[gatebox] (cm-gate) at (10.8,-1.0) {assert READY};

\node[step] (cm-stage) at (10.8,-2.4) {
  \textbf{git add --all}\\(when stage\_all=True)
};

\node[step] (cm-skip) at (10.8,-4.1) {
  skip if no staged\\changes
};

\node[step] (cm-commit) at (10.8,-5.8) {
  \textbf{git commit}\\
  each repo with staged\\changes; refresh SHA
};

\node[orderbox] (cm-order) at (10.8,-7.2) {leaf → parent → root};
...
\node[seclabel] (pu-title) at (16.1, 0) {\texttt{push\_tree}};

\node[gatebox] (pu-gate) at (16.1,-1.0) {assert READY};

\node[step] (pu-push) at (16.1,-3.25) {
  \textbf{git push}\\
...
\node[orderbox] (pu-order) at (16.1,-7.2) {leaf → parent → root};
...
\node[gatebox, minimum width=16.0cm]
    (footer) at (8.05,-8.2) {tree remains \textbf{READY} on success; \texttt{freeze} builds on add/commit/push/tag and emits a \texttt{.gts}};
...
\draw[sidarr] (pu-order.south) -- (footer.east);
