...
\node[orderbox, minimum width=5.0cm]
    (freeze-note) at (12.3,-8.2) {\texttt{commit}/\texttt{push}/\texttt{tag}/\texttt{freeze} preflight: detached HEAD, remotes, divergence, merges, submodules, snapshot SHA drift; \texttt{tag} also requires clean + absent tag; integration cycle validates CLI-first \texttt{add->commit->push->tag->freeze}};
