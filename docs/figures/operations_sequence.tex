...
\node[orderbox, minimum width=5.0cm]
    (freeze-note) at (12.3,-8.2) {\texttt{tag}/\texttt{freeze} preflight: remotes, branch align, tag absence, submodules; \texttt{tag} is clean + non-forcing; integration cycle validates CLI-first \texttt{add->commit->push->tag->freeze}};
