...
\node[state, right=of unloaded] (declared)  {LOADED};
...
\draw[trans] (unloaded) -- node[above, font=\scriptsize]{read(.cgs)} (declared);
\draw[trans] (declared) -- node[above, font=\scriptsize]{expand(.gts)} (pending);
...
\draw[trans, bend right=20] (ready.north) to node[above, font=\scriptsize]{freeze / next .gts id} (declared.north);
