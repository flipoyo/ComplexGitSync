...
\node[classbox] (client) {
  ...
  \nodepart{three}
  read(path) / load\_source(path)\\
  validate(path)\\
  format\_project\_tree()\\
  clone(path) / restart(path)\\
  checkout(ref, ref\_kind)\\
  add() / commit(message)\\
  push() / tag(name)\\
  freeze(name) / launch(path)
};
...
\caption{GitTree lifecycle state machine.  Every bootstrapping operation
  (\texttt{read}, \texttt{clone}, \texttt{restart/pull}, \texttt{launch}) must
  lead to a usable tree; synchronized Git operations (\texttt{checkout},
...
\draw[trans] (declared) -- node[above, font=\scriptsize]{clone / restart / launch} (pending);
...
\draw[trans, bend right=20] (ready.north) to node[above, font=\scriptsize]{read(.cgs)} (declared.north);
