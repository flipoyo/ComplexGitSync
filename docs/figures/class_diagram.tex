...
\begin{figure}[ht]
\centering
\caption{GitTree lifecycle state machine.  The user-facing lifecycle is
  \texttt{read(.cgs)} $\rightarrow$ \texttt{.gts LOADED}
  $\rightarrow$ \texttt{expand(.gts)} $\rightarrow$ \texttt{.gts PENDING}
  $\rightarrow$ \texttt{verify(.gts)} $\rightarrow$ \texttt{.gts READY};
...
\draw[trans] (unloaded) -- node[above, font=\scriptsize]{read(.cgs)} (loaded);
\draw[trans] (loaded) -- node[above, font=\scriptsize]{expand(.gts)} (pending);
\draw[trans] (pending)  -- node[above, font=\scriptsize]{verify(.gts)} (ready);
